...
- [ ] 時間割
  ...
  - [ ] show/hide機能
    - [ ] Listにだけ表示されるようにするか，もしくは時間割上に網線で表示される（+"非表示の授業は単位数にカウントされません"みたいなメッセージ）か
  - [ ] List形式で表示
  - [ ] Google Calendar / .icsエクスポート？
      - [ ] 授業開始日と終了日
      - [ ] 休講日を除く
- [x] 授業検索
  ...
  - [ ] Foundation, Area Major
    ...
    - [ ] SELECT * FROM courses WHERE course_code GLOB '[A-Z][A-Z][A-Z]1[0-9][0-9]'; のようにGLOBを使えばワイルドカードで100番台は持ってこられる
